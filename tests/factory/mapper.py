...
class ModelMapperFactoryTest(unittest.TestCase):
    ...
    def setUp(self, orig_model=None, dest_model=None, mapper=None):
        origin_model = get_origin_model() if orig_model is None else orig_model
        destination_model = get_destination_model() if dest_model is None else dest_model
        mapper = get_model_mapper() if mapper is None else mapper

        self._model_mapper = ModelMapper(origin_model, destination_model, mapper)
        self._model_mapper.prepare_mapper()

    @property
    def destination_model(self):
        return self._model_mapper.destination_model

    @property
    def origin_model(self):
        return self._model_mapper.origin_model

    @property
    def mapper(self):
        return self._model_mapper.mapper

    def update_destination_values(self):
        dest = self.destination_model
    ...
    def _assert_parent_uniform_list_model_values(self, d_link_index=0, assert_equal=True):
        orig_parent = self.origin_model['d'][d_link_index]
        dest_parent = self.destination_model.val_d
        assert_ = self._get_assert(assert_equal)
    ...
    def _assert_root_basic_values(self, assert_equal=True):
        ...
        assert_(self.origin_model['dd']['b'], self.destination_model.val_dd.val_b)
        assert_(self.origin_model['ddd']['a'], self.destination_model.val_ddd.val_a)
        assert_(self.origin_model['dddd'], self.destination_model.val_dddd)
        assert_(self.origin_model['complex'], self.destination_model.val_complex.get_val())
